...
- [ ] `review` maps to the exec review form with model, effort, and last-message output; no sandbox flag is passed
- [ ] `--uncommitted`, `--base`, `--commit` pass through; more than one is rejected
- [ ] No scope flag: uncommitted on a dirty tree, base against the default branch on a clean tree
- [ ] Focus on stdin reaches Codex as custom instructions
- [ ] Review reuses the run directory and final message plumbing
- [ ] Skill text has the adversarial instruction block and says when to use it
- [ ] Skill text says: present findings, then stop; never apply a fix before the user chooses
- [ ] Tests cover scope flags, mutual exclusion, the dirty and clean defaults, and absence of any sandbox flag
